...
<div id="salesRecord">
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: requisitions.length > 0 ? '' : 'none'}">
			...
			<div class="table-responsive" id="reportContent">
				<table class="record-table" v-if="recordType == 'with_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'with_details' ? '' : 'none'}">
					...
					<tbody style="text-align: center;">
						<template v-for="sale in requisitions">
							<tr>
				...
				<table class="record-table" v-else style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
					...
					<tbody style="text-align: center;">
						<tr v-for="sale in requisitions">
							<td>{{ sale.requisition_no }}</td>
							<td>{{ sale.date }}</td>
							<td>{{ sale.total_quantity }}</td>
							<td>{{ sale.User_Name }}</td>
							<td>{{ sale.purchase_order_status == 'pending' ? 'Pending' : 'Complete' }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:center;">{{ requisitions.reduce((prev, curr)=>{return prev + parseFloat(curr.total_quantity)}, 0) }}</td>
							<td></td>
							<td></td>
							<td></td>
						</tr>
	...
	new Vue({
		...
		methods: {
			...
			getSalesRecord() {
				...
				let url = '/get-requisition-report';
