...
<div id="salesRecord">
    <div class="row" style="border-bottom: 1px solid #ccc;padding: 3px 0;">
        <div class="col-md-12">
            <form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
                ...
                <div class="form-group" style="display:none;"
                    v-bind:style="{display: searchType == 'vehicle' && vehicles.length > 0 ? '' : 'none'}">
                    <label>Vehicle</label>
                    <v-select v-bind:options="vehicles" v-model="selectedVehicle" label="vehicle_reg_no"></v-select>
                </div>

                <div class="form-group" style="display:none;"
                    v-bind:style="{display: searchType == 'workshop' ? '' : 'none'}">
    ...
    <div class="row" style="margin-top:15px;display:none;"
        v-bind:style="{display: dataRecords.length > 0 ? '' : 'none'}">
        ...
        <div class="col-md-12" id="dataTable">
            <div class="table-responsive" id="reportContent">
                ...
                <table class="record-table" v-if="recordType == 'with_details'" style="display:none"
                        ...
                        <template v-for="sale in dataRecords">
                            <tr>
                                ...
                                <td style="text-align:center;">
                                    <a href="" title="Requisition Form"
                                        v-bind:href="`/requisition_print/${sale.requisition_id}`" target="_blank"><i
                                    ...
                                    <a v-if="sale.purchase_order_status == 'pending'" href="" title="Delete Sale"
                                        @click.prevent="deleteRequisition(sale.requisition_id)"><i
                ...
                <table class="record-table" v-else style="display:none"
                    ...
                    <tbody style="text-align: center;">
                        <tr v-for="item in dataRecords">
                            ...
                            <td>
                                <a v-if="item.image1"
                                    :href="<?php base_url() ?>'/uploads/maintanance_img/' + item.image1"
                                    target="_blank">Image1</a> <br>
                                <a v-if="item.image2"
                                    :href="<?php base_url() ?>'/uploads/maintanance_img/' + item.image2"
                                    target="_blank">Image2</a>
                            </td>

                            <td style="text-align:center;">
                                <a href="" title="Invoice" v-bind:href="`/maintanance_print/${item.maintenance_id}`"
                                ...
                                <?php if ($this->session->userdata('accountType') != 'u') { ?>
                                <a href="" title="Edit" v-bind:href="`/daily-maintenance/${item.maintenance_id}`"><i
                                        class="fa fa-edit"></i></a>
                                <a href="" title="Delete" @click.prevent="deleteItem(item.maintenance_id)"><i
                                        class="fa fa-trash"></i></a>
                                <?php } ?>
                            </td>
                        </tr>
...
<script src="<?php echo base_url(); ?>assets/js/excel-export/jquery.base64.js"></script>
<script src="<?php echo base_url(); ?>assets/js/excel-export/tableExport.jquery.json"></script>
<script src="<?php echo base_url(); ?>assets/js/excel-export/tableExport.js"></script>
    ...
    methods: {
        ...
        getSalesRecord() {
            ...
            let filter = {
                ...
                workshop_id: this.selectedWorkshop == null || this.selectedWorkshop.workshop_id == '' ? '' :
