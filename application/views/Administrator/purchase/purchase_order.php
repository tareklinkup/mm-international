...
<div class="row" id="purchase">
	...
	<div class="col-xs-12">
					...
					<div class="row">
						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									<label class="col-xs-4 control-label no-padding-right"> Requisition No </label>
									<div class="col-xs-7">
										<v-select v-bind:options="requisitions" v-model="selectedRequisition" label="requisition_no" v-on:input="getRequisitionData" placeholder="Select---" :disabled="purchase.purchaseId == 0 ? false : true"></v-select>
									</div>
									<div class="col-xs-1" style="padding: 0;">
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							<td>{{ sl + 1}}</td>
							<td>{{ product.vehicle_reg_no }} - {{ product.Product_Name }}</td>
							<td>{{ product.quantity }}</td>
							<td>
								<input style="text-align: center;" type="number" v-model="product.unit_price" v-on:input="calTotal(sl)" class="form-control">
							</td>
		...
		<div class="row text-right">
			<button style="display:none;" :style="{display: cart.length > 0 ? '' : 'none'}" class="btn btn-success" style="margin: 10px 12px 0px 0px" :disabled="btnStatus" v-on:click="savePurchaseOrder">Save Purchase Order</button>
		</div>
	</div>
...
<script>
	...
	new Vue({
		...
		methods: {
			getRequisitins() {
				axios.post('/get-requisition-report', {
					purchase_order: 'yes'
				}).then(res => {
					// console.log(res);
			...
			getRequisitionData() {
				if (this.purchase.purchaseId == 0) {
					this.cart = [];
					let id = this.selectedRequisition.requisition_id;
					this.requisitions.forEach(ele => {
						if (ele.requisition_id == id) {
							ele.requisitionDetails.forEach(element => {
								element.unit_price = '';
			...
			calTotal(index) {
				this.cart[index].total_amount = (parseFloat(this.cart[index].unit_price) * parseFloat(this.cart[index].quantity)).toFixed(2);
				...
				this.purchase.sumTotal = sum.toFixed(2);
			},
			savePurchaseOrder() {
				...
				if (this.purchase.sumTotal == 0) {
					alert('Sum Total cannot be zero');
					return;
				}

				let details = {
					requisition_id: this.selectedRequisition.requisition_id,
					supplier_id: this.selectedSupplier.Supplier_SlNo,
					purchaseOrder_invoiceNo: this.purchase.invoice,
					purchaseOrder_date: this.purchase.purchaseDate,
					total_price: this.purchase.sumTotal,
				}
				let url = "/save-purchase-order";
				if (this.purchase.purchaseId != 0) {
					url = "/update-purchase-order";
					details.purchaseOrder_id = this.purchase.purchaseId;
				}
			...
			getPurchaseOrder() {
				let filter = {
					purchaseOrder_id: this.purchase.purchaseId,
				}
				axios.post("/get-all-purchase-Order", filter).then(res => {
					// console.log(res.data.purchaseOrderDetails);
					this.selectedSupplier = {
						Supplier_SlNo: res.data.purchaseOrder[0].supplier_id,
						Supplier_Name: res.data.purchaseOrder[0].Supplier_Name,
						Supplier_Mobile: res.data.purchaseOrder[0].Supplier_Mobile,
						Supplier_Address: res.data.purchaseOrder[0].Supplier_Address,
						display_name: res.data.purchaseOrder[0].Supplier_Code + ' - ' + res.data.purchaseOrder[0].Supplier_Name,
					};
					this.selectedRequisition = {
						requisition_id: res.data.purchaseOrder[0].requisition_id,
						requisition_no: res.data.purchaseOrder[0].requisition_no,
					}
