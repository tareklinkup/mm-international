			...
			<?php if ($selected) { ?>
				<form class="form-vertical" method="post" enctype="multipart/form-data" action="<?php echo base_url(); ?>company_profile_Update">
						...
						<div class="col-sm-12">
							<div class="center">
								<?php if ($selected->Company_Logo_thum != "") { ?>
									<img id="hideid" src="<?php echo base_url() . 'uploads/company_profile_thum/' . $selected->Company_Logo_thum; ?>" alt="" style="width:60px;border:1px solid #ccc">
								<?php } else { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-12">
							<div class="center">
								<?php if ($selected->iso_image != "") { ?>
									<img id="hideidIso" src="<?php echo base_url() . 'uploads/\company_profile_org/' . $selected->iso_image; ?>" alt="" style="width:60px;border:1px solid #ccc">
								<?php } else { ?>
									<img id="hideidIso" src="<?php echo base_url(); ?>images/No-Image-.jpg" alt="" style="width:200px;">
								<?php } ?>
								<img id="previewIso" src="#" style="width:100px;height:100px" hidden>
								<h6>Preview</h6>
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						<label class="control-label" for="companyIso">Change ISO</label>
						<div>
							<input name="companyIso" id="companyIso" type="file" onchange="readURLIso(this)" class="form-control" style="height:35px;" />
						</div>
					</div>
...
<script type="text/javascript">
	...
	function readURLIso(input) {
		if (input.files && input.files[0]) {
			var reader = new FileReader();
			reader.onload = function(e) {
				document.getElementById('previewIso').src = e.target.result;
			}
			reader.readAsDataURL(input.files[0]);
			$("#hideidIso").hide();
			$("#previewIso").show();
...
<script type="text/javascript">
	function Employee_submit() {
		...
		fd.append('companyIso', $('#companyIso')[0].files[0]);
