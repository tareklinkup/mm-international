...
class Model_Table extends CI_Model
{
    ...
    public function generatePurchaseInvoice()
    {
        ...
        $purchases = $this->db->query("select purchaseOrder_invoiceNo from tbl_purchase_order order by purchaseOrder_invoiceNo desc limit 1")->row();
        if ($purchases) {
            $newPurchaseId = $purchases->purchaseOrder_invoiceNo + 1;
        } else {
            $newPurchaseId = date('Y') . "000001";
        }
    ...
    public function productStock($parts_id)
    {
        $stockQuery = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$parts_id, $this->session->userdata("BRANCHid")]);
