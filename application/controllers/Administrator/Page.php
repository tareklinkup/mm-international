...
class Page extends CI_Controller
{
    ...
    {
        if ($_FILES['companyIso']) {
            $filename = $_FILES['companyIso']['name'];
            $arr = explode('.', $filename);
            $extension = end($arr);
            $newName =  'iso_image.' . $extension;

            move_uploaded_file($_FILES["companyIso"]["tmp_name"], 'uploads/company_profile_org/' . $newName);

            $data = [];
            $data['iso_image'] = 'iso_image.' . $extension;
