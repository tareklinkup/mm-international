...
class Purchase extends CI_Controller
{
    ...
    public function purchaseEdit($purchaseId)
    {
        ...
        $data['invoice'] = $this->db->query("select purchaseOrder_invoiceNo from tbl_purchase_order where purchaseOrder_id = ?", $purchaseId)->row()->purchaseOrder_invoiceNo;
        $data['content'] = $this->load->view('Administrator/purchase/purchase_order', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            $invoice = $data->masterData->purchaseOrder_invoiceNo;
            $invoiceCount = $this->db->query("select * from tbl_purchasemaster where PurchaseMaster_InvoiceNo = ?", $invoice);
            // if ($invoiceCount != 0) {
            //     $invoice = $this->mt->generatePurchaseInvoice();
            // }
            ...
            $total_amt = $invoiceCount->row()->PurchaseMaster_TotalAmount + $data->masterData->purchaseTotalAmount;

            if ($invoiceCount->num_rows()) {

                $purchase = array(
                    'PurchaseMaster_TotalAmount'    => $total_amt,
                    'PurchaseMaster_SubTotalAmount' => $total_amt,
                    'PurchaseMaster_PaidAmount'     => $total_amt,
                    'UpdateBy' => $this->session->userdata("FullName"),
                    'UpdateTime' => date('Y-m-d H:i:s'),
                    'PurchaseMaster_BranchID' => $this->session->userdata('BRANCHid')
                );
                $this->db->where('PurchaseMaster_InvoiceNo', $invoice)->update('tbl_purchasemaster', $purchase);
                $purchaseId = $invoiceCount->row()->PurchaseMaster_SlNo;
            } else {
                $purchase = array(
                    'Supplier_SlNo'                 => $data->masterData->supplier_id,
                    'PurchaseMaster_InvoiceNo'      => $data->masterData->purchaseOrder_invoiceNo,
                    'PurchaseMaster_OrderDate'      => $data->masterData->purchaseOrder_date,
                    'PurchaseMaster_PurchaseFor'    => '',
                    'PurchaseMaster_TotalAmount'    => $data->masterData->purchaseTotalAmount,
                    'PurchaseMaster_DiscountAmount' => 0,
                    'PurchaseMaster_Tax'            => 0,
                    'PurchaseMaster_Freight'        => 0,
                    'PurchaseMaster_SubTotalAmount' => $data->masterData->purchaseTotalAmount,
                    'PurchaseMaster_PaidAmount'     => $data->masterData->purchaseTotalAmount,
            ...
            foreach ($data->cartProducts as $product) {
                if ($product->received_qty != '' || $product->received_qty != 0) {
                    $purchaseDetails = array(
                        'PurchaseMaster_IDNo'           => $purchaseId,
                        'Product_IDNo'                  => $product->parts_id,
                        'PurchaseDetails_TotalQuantity' => $product->received_qty,
                        'PurchaseDetails_Rate'          => $product->unit_price,
                        'PurchaseDetails_TotalAmount'   => $product->received_qty * $product->unit_price,
                        'PurchaseDetails_Note'          => $product->remarks,
                        'Status'                        => 'a',
                        'AddBy'                         => $this->session->userdata("FullName"),
                        'AddTime'                       => date('Y-m-d H:i:s'),
                    ...
                    $previousStock = $this->mt->productStock($product->parts_id);

                    $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$product->parts_id, $this->session->userdata('BRANCHid')])->num_rows();
                    if ($inventoryCount == 0) {
                        $inventory = array(
                            'product_id' => $product->parts_id,
                            'purchase_quantity' => $product->received_qty,
                        ...
                        $this->db->query("
                            ...
                            where product_id = ? 
                            and branch_id = ?
                        ", [$product->received_qty, $product->parts_id, $this->session->userdata('BRANCHid')]);
                    }
                }
            ...
            $this->db->query("update tbl_purchase_order set received_status = 'complete' where purchaseOrder_id = ?", [$product->purchaseOrder_id]);
