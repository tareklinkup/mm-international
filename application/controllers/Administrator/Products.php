...
<?php
    ...
    public function updateCurrentStock()
    {
        ...
        $data['parts_id']    = $obj->productId;
        $data['quantity']    = $obj->newQty;
        $data['status']    = 'a';
        $data['AddBy']     = $this->session->userdata("userId");
        $data['AddTime']   = date('Y-m-d H:i:s');
        $data['branch_id'] = $this->brunch;

        $this->db->insert('tbl_wastage_restore', $data);

        // stock update
        $this->db->query("UPDATE tbl_currentinventory 
        set wastage_qty = wastage_qty - ?
        where product_id = ?
        and branch_id = ?
        ", [$obj->newQty, $obj->productId, $this->session->userdata('BRANCHid')]);

        $this->db->query("UPDATE tbl_currentinventory 
        set wastage_restore_qty = wastage_restore_qty + ?
        where product_id = ?
        and branch_id = ?
        ", [$obj->newQty, $obj->productId, $this->session->userdata('BRANCHid')]);


        echo json_encode(['success' => true, 'message' => 'Data save successfully']);
    }
}
